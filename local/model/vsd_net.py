...
class Visual_VAD_Conformer_Net(nn.Module):
    ...
    def forward(self, video_inputs, current_frame, return_embedding=False):
        #print("videoinput:", video_inputs.shape)
        lip_encoder = self.lip_encoder(video_inputs) #(B, T, 256)
        batchsize, Time, _ = lip_encoder.shape
        #print("b,t,256",lip_encoder.shape)
        ...
        #print("conformer:", conformer3.shape)
        ...
        lens = [k for i, m in enumerate(current_frame) for k in range(i * Time, m + i * Time)]
        #print("decoder.resape0:", decoder.reshape(decoder.shape[0] * decoder.shape[1], -1).shape)
        #print("decoder.resape:", decoder.reshape(decoder.shape[0] * decoder.shape[1], -1)[lens, :].shape)

        fc1 = self.fc1(decoder.reshape(-1, 256)[lens, :])
        fc2 = self.fc2(fc1)
