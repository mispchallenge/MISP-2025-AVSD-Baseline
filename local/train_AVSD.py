...
def main(args):
    ...
    # define the model
    nnet = AIVECTOR_ConformerVEmbedding_SD(config.configs_SC_Multiple_8Speakers_AEmbedding_VEmbedding_2Classes)
    #nnet = nnet.cuda()
    ...
    nnet = torch.nn.DataParallel(nnet, device_ids = [0, 1, 2]).cuda()
    ...
    for iter_ in range(args.start_iter, args.end_iter):
        ...
        running_loss = 0.0
        nnet.train()
        dataset_train = Audio_AEmbedding_VEmbedding_Worse_Data_Reader(args.train_audio_fea_dir, args.train_speaker_embedding, args.train_video_fea_scp, label_2classes_train, set_video_silence=0.15, set_video_speak=0.20, min_speaker=2, max_speaker=8, max_utt_durance=800, frame_shift=600, audio_type=args.audio_type.split("+"))
