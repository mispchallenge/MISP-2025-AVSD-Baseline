...
class Audio_AEmbedding_Video_Worse_Data_Decode_Reader():
        ...
        with open(video_scp) as SCP_IO:
            for l in SCP_IO:
                ...
                if speaker == "999":
                    continue
                try:
                    a = self.label.frame_label[session][speaker]
                except:
                    #print(f"get_silent_video_segments: {session}{speaker} not in label")
                    #break
                    continue
                start = int(st
                start = int(start)
    ...
    def get_slience_video(self, durance):
        ...
        session = os.path.basename(video_path).split("-")[0]
    ...
    def get_video_feature(self, session, speakers, start, end, sample_video_path):
        ...
        for id, spk in enumerate(speakers):
            ...
            if cur_video_path == []:  #total silence
                ...
                tmp_session = os.path.basename(cur_video_path[0][0]).split("-")[0]
...
class Audio_AEmbedding_VEmbedding_Worse_Data_Reader():
    def __init__(self, audio_dir, aembedding_scp, vembedding_scp, label, set_video_silence=0, set_video_speak=0, differ_speaker=True, min_speaker=0, max_speaker=6, max_utt_durance=800, frame_shift=None, mixup_rate=0, alpha=0.5, audio_type=["RAW"]):
        self.max_utt_durance = max_utt_durance
        if frame_shift == None:
            self.frame_shift = self.max_utt_durance // 2
        else:
        ...
        self.silent_video_segments = self.get_silent_video_segments(vembedding_scp)
        self.speech_video_segments = self.get_speech_video_segments(vembedding_scp)
        self.normal, self.worse_silence, self.worse_speech = 0, 0, 0
        self.same_session_feature_list, self.feature_list = self.get_feature_info(audio_dir, vembedding_scp, audio_type)

    def get_silent_video_segments(self, video_scp):
        silent_video_segments = []
        with open(video_scp) as SCP_IO:
            for l in SCP_IO:
                session = os.path.basename(l).split("_")[0]
                speaker, start, end = os.path.basename(l).split("_")[-1].split(".")[0].split("-")
                if speaker == "999":
                    continue
                try:
                ...
                if np.sum(self.label.frame_label[session][speaker][start*4:end*4][:, 1]) < 0.01 * self.max_utt_durance:
                    silent_video_segments.append(l.rstrip())
        print(f"silent video segments: {len(silent_video_segments)}")
        np.random.shuffle(silent_video_segments)
        return silent_video_segments
    
    def get_speech_video_segments(self, video_scp):
        speech_video_segments = []
        with open(video_scp) as SCP_IO:
            for l in SCP_IO:
                session = os.path.basename(l).split("_")[0]
                ...
                if np.sum(self.label.frame_label[session][speaker][start*4:end*4][:, 1]) > 0.90 * self.max_utt_durance:
                    speech_video_segments.append(l.rstrip())
        print(f"speech video segments: {len(speech_video_segments)}")
        np.random.shuffle(speech_video_segments)
        return speech_video_segments

    def worse_data(self, item):
        #item = [session, utt, embeddings[session][utt], os.path.join(audio_dir, "misp2021_train_middle_raw_cmn_slide", f"{session}_Middle_0_RAW.fea")]
        session, utt, embeddings, audio_path = item
        start, end = utt.split('-')
        start, end = int(start), int(end)
        ...
        if choose < self.set_video_silence:
            ...
            if max_speech >= 0.8 * (end - start):
                for i in range(len(embeddings)):
                    if embeddings[i][0] == speaker:
                        embeddings[i][1] = self.silent_video_segments[self.worse_silence % int(len(self.silent_video_segments))]
                        self.worse_silence += 1
                        return [session, utt, embeddings, audio_path]
            ...
            if min_speech <= 0.2 * (end - start):
                for i in range(len(embeddings)):
                    if embeddings[i][0] == speaker:
                        embeddings[i][1] = self.speech_video_segments[self.worse_speech % int(len(self.speech_video_segments))]
                        self.worse_speech += 1
                        return [session, utt, embeddings, audio_path]
        self.normal += 1
        return [session, utt, embeddings, audio_path]

    def get_feature_info(self, audio_dir, video_scp, audio_type):
        feature_list = []
        same_session_feature_list = {}
        embeddings = {}
        speakers = {}
        with open(video_scp) as SCP_IO:
            for l in SCP_IO:
                ...
                try:
                    ...
                    #break
                    continue
                if session not in embeddings.keys():
                    embeddings[session] = {}
                    speakers[session] = []
                if speaker not in speakers[session]:
                    speakers[session].append(speaker)
                utt = f"{start}-{end}"
                if utt not in embeddings[session].keys():
                    embeddings[session][utt] = []
                embeddings[session][utt].append([speaker, l.rstrip()])
        
        for session in tqdm.tqdm(embeddings.keys()):
            try:
                MIN_LEN = self.label.get_session_length(session) // 4
                for s in audio_type:
                    for ch in range(1, 9):
                        total_frame = HTK.readHtk_info(os.path.join(audio_dir, f"{session}_{s}_{ch}.fea"))[0] // 4 
                        MIN_LEN = min(MIN_LEN, total_frame)
            except:
                print(l.rstrip())
                continue
            ...
            if num_speaker != self.label.mixture_num_speaker(session):
                ...
                continue
            same_session_feature_list[session] = []
            for utt in embeddings[session].keys():
                if int(utt.split('-')[1]) > MIN_LEN:
                    continue
                if len(embeddings[session][utt]) != num_speaker:
                    print(f"len(embeddings[session][utt]){len(embeddings[session][utt])} != num_speaker{num_speaker}")
                for s in audio_type:
                    for ch in range(1, 9):
                        same_session_feature_list[session].append(self.worse_data([session, utt, embeddings[session][utt], os.path.join(audio_dir, f"{session}_{s}_{ch}.fea")]))
            feature_list.extend(same_session_feature_list[session])
        total_item = self.normal + self.worse_speech + self.worse_silence
        print(total_item)
        print(f"Normal:{self.normal / total_item} Set_silence:{self.worse_silence / total_item} Set_speak:{self.worse_speech / total_item}")
    ...
    def __getitem__(self, idx):
        ...
        session, utt, video_embeddings, audio_path = self.feature_list[idx]
        real_session = os.path.basename(audio_path).split(".")[0]
        start, end = utt.split('-')
        start, end = int(start), int(end)
        _, audio_fea = self.load_fea(audio_path, start*4, end*4)
        ...
        for k in video_embeddings:
            video_embedding_path[k[0]] = k[1]
        for spk in speakers:
            try:
                video_embedding.append(self.load_vembedding(video_embedding_path[spk]))
            except:
                silent_video = self.silent_video_segments[self.worse_silence % int(len(self.silent_video_segments))]
                self.worse_silence += 1
                video_embedding.append(self.load_vembedding(silent_video))
        ...
        for l in np.random.choice(range(len(self.silent_video_segments)), self.max_speaker - num_speaker, replace=False):
            video_embedding_path = self.silent_video_segments[l]
            video_embedding.append(self.load_vembedding(video_embedding_path))
        video_embedding = np.stack(video_embedding)
        ...
        if num_speaker > self.max_speaker:
            ...
            video_embedding = video_embedding[speaker_index][:self.max_speaker]
            mask_label = mask_label[speaker_index][:self.max_speaker]
        '''
        returns:
        data: [T, F]
        audio_embedding: [num_speaker, video_embedding]
        video_embedding: [num_speaker, T, video_embedding]
        mask_label: [num_speaker, T, C]
        '''
        return audio_fea, audio_embedding, video_embedding, mask_label
