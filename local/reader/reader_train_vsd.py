    ...
    def get_feature_info(self, feature_scp):
        ...
        with open(feature_scp) as SCP_IO:
            for l in SCP_IO:
                ...
                B344-PSCx3-240_999-36475-36499.htk
                '''
...
def myCollateFn(sample_batch):
    ...
    #print("fea0", np.array(data_feature).shape)
    try:
        if not data_feature:
            raise ValueError("data_feature is empty. Skipping pad_sequence.")
        data_feature = pad_sequence(data_feature, batch_first=True, padding_value=0.0)
    except ValueError as e:
        print(e)  # 输出异常信息，表示跳过 pad_sequence 操作
    #data_feature = pad_sequence(data_feature, batch_first=True, padding_value=0.0)
    #print("fea1", np.array(data_feature).shape)
