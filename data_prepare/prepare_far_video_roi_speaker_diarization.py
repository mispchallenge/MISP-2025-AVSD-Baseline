...
def segment_video_roi_json(video_path, roi_json_path, roi_store_dir):
    ...
    speaker = roi_json_path.split('/')[-1].split('.')[0]
...
def process(video_item, video_dir, roi_json_dir, roi_store_dir):
    for i in video_item:
        ...
        matching_json_files = [f for f in os.listdir(roi_json_dir) if f.split('_')[0] == i and f.endswith(".json")]
        if os.path.isfile(video_path) and matching_json_files:
            roi_json_path = os.path.join(roi_json_dir, matching_json_files[0])
